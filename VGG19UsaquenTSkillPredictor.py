...
imageFeatures = genfromtxt('VGG19Usaquen_Ftrs.csv', delimiter=',')
...
voteSetFile = 'usaquenSRandomVote4Predict_SchemIII.txt'
...
with tf.Session() as sess:
    ...
    # for i in range(0, votesIndex.shape[0]-ImgVtTestSize):
        ...
        evalDiff = resultSftMx[0][0] - resultSftMx[0][1]
        if evalDiff < 0:
            evalDiff = evalDiff*(-1)
    ...
    np.savetxt("randomIndexerS_UsaquenPredict4TSkill.csv", votesIndex, delimiter=",", fmt='%i')
    
    
    print("________________________________Building TrueSkill Output File")
    mu, sigma, fileName, zeroVotes, oneVotes, twoVotes, totalVotes = actualOneMatchSycleVoteTrueSkillExplorer(randomIndexerSPath = 'randomIndexerS_UsaquenPredict4TSkill.csv',
                                                                                                          imgSetSize = 9478,
                                                                                                          zoneName = "Usaquen")
